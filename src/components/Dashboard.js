...
function Dashboard() {
  ...
  const updateStake = () => {
    fetch('http://localhost:3001/api/config/stake', {
      ...
      .then(response => response.json())
      .then(data => {
        setResponses(prev => [...prev, `Stake updated: ${data.stake}`]);
      })
  ...
  const updateStopLoss = () => {
    fetch('http://localhost:3001/api/config/stopLoss', {
      ...
      .then(response => response.json())
      .then(data => {
        setResponses(prev => [...prev, `Stop loss updated: ${data.stopLoss}`]);
      })
  ...
  const updateTargetProfit = () => {
    fetch('http://localhost:3001/api/config/targetProfit', {
      ...
      .then(response => response.json())
      .then(data => {
        setResponses(prev => [...prev, `Target profit updated: ${data.targetProfit}`]);
      })
      .catch(error => console.error('Error updating target profit:', error));
  };

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#0003' }}>
      <AppBar position="static">
     
      </AppBar>
      <Container component="main" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', padding: 0 }}>
        <Grid container spacing={0} sx={{ flexGrow: 1, height: '100%', padding: 0 }}>
          <Grid item xs={12} md={9} sx={{ height: '75%', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ flexGrow: 1, backgroundColor: '#0003', padding: 2 }}>
              <Typography variant="h5" component="h2" gutterBottom sx={{ color: '#fff' }}>
                Gráfico de Tick
              </Typography>
              <Box sx={{ height: '100%', border: '1px solid #444', flexGrow: 1, overflow: 'hidden', padding: 0 }}>
                <Line 
                  data={chartData} 
                  options={{
                    plugins: {
